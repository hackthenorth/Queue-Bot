...
export class ParsingUtils {
  ...
  public static async reportChannelNotFound(
    ...
    if (channels.length === 0) {
      ...
      if (channels.length === 1) {
        ...
        response +=
          channels[0].name + (includeMention ? " @{user}" : "") + "`.";
      } else {
        // Multiple channels, list them
        response +=
          `{${target} name}` + (includeMention ? " @{user}" : "") + "`.";
        if (isAQueue) {
          response +=
            "\nAvailable " +
            (type ? `**${type}** ` : "") +
            `queues: ${channels.map((channel) => " `" + channel.name + "`")}.`;
        }
      }
    }
  ...
  public static getChannel(
    ...
    if (availableChannels.length > 0) {
      ...
      const channels = type
        ? availableChannels.filter((channel) => channel.type === type)
        : availableChannels;
      ...
      if (channels.length === 1) {
        ...
        if (channel) {
          ...
          if (
            channels.map((ch) => ch.id).includes(parsed.message.channel.id) &&
            type !== "voice" &&
            !parsed.arguments
          ) {
            return parsed.message.channel as TextChannel;
          } else {
            this.reportChannelNotFound(
              parsed,
              channels,
              includeMention,
              true,
      ...
      SchedulingUtils.scheduleResponseToMessage(
        `No queue channels set.\n` +
          `Set a queue first using \`${parsed.queueGuild.prefix}${
            Base.getCmdConfig().queueCmd
          } {channel name}\`.`,
        parsed.message
      );
  ...
  public static async getStoredChannel(
    ...
    const storedChannels = await QueueChannelTable.getFromGuild(
      parsed.message.guild
    );
